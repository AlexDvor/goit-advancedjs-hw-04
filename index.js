...
`};let p=null;const a={form:document.querySelector(".form"),galleryList:document.querySelector(".gallery"),loader:document.querySelector(".loader"),loadMoreBtn:document.querySelector(".js-load-more-btn"),card:document.querySelector(".gallery-item")},l=(t,e)=>{e==="error"?m.error({message:t,position:"topRight"}):e==="warning"?m.warning({message:t,position:"topRight"}):m.show({message:t,position:"topRight"})},f=()=>{a.loadMoreBtn.classList.add("inactive"),a.loadMoreBtn.removeEventListener("click",S)},F=()=>{a.loadMoreBtn.classList.remove("inactive"),a.loadMoreBtn.addEventListener("click",S)},H=()=>{const t=a.galleryList.children[0],{height:e}=t.getBoundingClientRect();return e},M=()=>{p?p.refresh():p=new x(".gallery-link",{captionsData:"alt",captionDelay:250,overlayOpacity:.9})},S=async t=>{c.incrementPage();try{const{hits:e,totalHits:o}=await c.getPhotoByQuery(c.queryField);if(e.length===0&&o===0){l("Sorry, there are no images matching your search query. Please try again!","error");return}const i=e.map(s=>q(s)).join("");a.galleryList.insertAdjacentHTML("beforeend",i);const r=a.galleryList.children.length;window.scrollBy({top:H()*2,left:0,behavior:"smooth"}),M(),o===r&&(l("We're sorry, but you've reached the end of search results.","warning"),f())}catch(e){l(e.message,"error"),console.log(e)}},T=async t=>{t.preventDefault();const e=t.target,i=new FormData(e).get("search-field").trim();if(!i){l("The search field is empty. Please enter text to search.","warning");return}c.resetPage(),a.loader.classList.add("is-loaded");try{const{hits:r,totalHits:s}=await c.getPhotoByQuery(i);if(r.length===0){l("Sorry, there are no images matching your search query. Please try again!","error");return}const n=r.map(E=>q(E)).join("");a.galleryList.innerHTML=n;const u=a.galleryList.children.length;s===u?f():F(),M()}catch(r){l(r.message,"error"),a.galleryList.innerHTML="",f(),console.log(r)}finally{a.loader.classList.remove("is-loaded")}};a.form.addEventListener("submit",T);
